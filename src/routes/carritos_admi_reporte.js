...
router.get('/carritos_admi_reporte/:id', isAuthenticated, function(req, res, next) {
...
router.get('/carrito_admi_reporte', isAuthenticated, function(req, res, next) {
    if (!req.session.carrito) { //si no hay nada en carrito de compras
        return res.render('carritos/carritos_admi_reporte', { Libros: null });
    }
    var carrito = new Carrito(req.session.carrito);
    res.render('carritos/carritos_admi_reporte', {

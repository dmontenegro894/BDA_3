...
router.get('/carritos_admi/:id', isAuthenticated, function(req, res, next) {
...
router.get('/carrito_admi', isAuthenticated, function(req, res, next) {
    if (!req.session.carrito) { //si no hay nada en carrito de compras
        return res.render('carritos/carritos_admi', { Libros: null });
    }
    var carrito = new Carrito(req.session.carrito);
    res.render('carritos/carritos_admi', {
